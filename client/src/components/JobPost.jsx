    ...
    <div className="w-full max-w-3xl bg-emerald-100 flex flex-col justify-center rounded-2xl shadow-xl hover:shadow-2xl transition-shadow duration-300">
      <form onSubmit={handleSubmit} className="flex flex-col p-10 gap-y-4 text-gray-700">
        <div className="flex flex-col gap-1">
          <label>Job Title</label>
          <input type="text" name="jobtitle" placeholder="Enter job title" onChange={handleChange} required value={jobData.jobtitle} className="border-blue-300 border-2 p-2 rounded" />
        </div>

        <div className="flex flex-col gap-1">
          <label>Description</label>
          <textarea name="description" placeholder="Enter job description" onChange={handleChange} required value={jobData.description} className="border-blue-300 border-2 p-2 rounded h-24 resize-none" />
        </div>

        <div className="flex flex-col gap-1">
          <label>Skills Required (comma-separated)</label>
          <input type="text" name="skills" placeholder="e.g., Web Development, Graphic Design" required value={skillsInput || ""} onChange={handleChange} className="border-blue-300 border-2 p-2 rounded" />
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div className="flex flex-col gap-1">
            <label>Minimum Experience</label>
            <select name="experience" onChange={handleChange} value={jobData.experience} required className="border-blue-300 border-2 p-2 rounded">
              <option value="">Select</option>
              <option value="Fresher">Fresher</option>
              <option value="1-3 years">1-3 Years</option>
              <option value="4-8 years">4-8 Years</option>
          ...
          <div className="flex flex-col gap-1">
            <label>Job Type</label>
            <select name="jobtype" onChange={handleChange} value={jobData.jobtype} required className="border-blue-300 border-2 p-2 rounded">
              <option value="">Select</option>
              <option value="Part-Time">Part-Time</option>
              <option value="Full-Time">Full-Time</option>
              <option value="Freelance">Freelance</option>
          ...
          <div className="flex flex-col gap-1">
            <label>Location Type</label>
            <select name="locationtype" onChange={handleChange} value={jobData.locationtype} required className="border-blue-300 border-2 p-2 rounded">
              <option value="">Select</option>
              <option value="On-Site">On-Site</option>
              <option value="Remote">Remote</option>
            </select>
          ...
          {jobData.locationtype === 'On-Site' && (
            <div className="flex flex-col gap-1">
              ...
              <input type="text" name="location" placeholder="City, State" onChange={handleChange} value={jobData.location} className="border-blue-300 border-2 p-2 rounded" />
            </div>
          )}
        </div>

        <fieldset className="border border-blue-300 p-4 rounded space-y-3 mt-4">
          <legend className="text-blue-500 font-semibold">Compensation</legend>

          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
              <label>Min Amount</label>
              <input type="number" name="compensation.minAmount" required onChange={handleChange} value={jobData.compensation.minAmount} className="border-blue-300 border-2 p-2 rounded" />
            </div>

            <div className="flex flex-col gap-1">
              <label>Max Amount</label>
              <input type="number" name="compensation.maxAmount" required onChange={handleChange} value={jobData.compensation.maxAmount} className="border-blue-300 border-2 p-2 rounded" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
              <label>Currency</label>
              <select name="compensation.currency" onChange={handleChange} value={jobData.compensation.currency} required className="border-blue-300 border-2 p-2 rounded">
                <option value="INR">INR</option>
                <option value="USD">USD</option>
                <option value="EUR">EUR</option>
              </select>
            ...
            <div className="flex flex-col gap-1">
              <label>Period</label>
              <select name="compensation.period" onChange={handleChange} value={jobData.compensation.period} required className="border-blue-300 border-2 p-2 rounded">
                <option value="">Select</option>
                <option value="Hourly">Hourly</option>
                <option value="Daily">Daily</option>
                <option value="Weekly">Weekly</option>
        ...
        <div className="flex flex-col gap-1 mt-4">
          <label>Deadline</label>
          <input type="date" name="deadline" min={new Date().toISOString().split('T')[0]} required value={jobData.deadline} onChange={handleChange} className="border-blue-300 border-2 p-2 rounded" />
        </div>

        <button type="submit" className="mt-6 w-[40%] mx-auto bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 rounded-md shadow-md transition">
          Post Job
        </button>
      </form>

      <p className="m-auto mb-6 text-xl font-semibold text-blue-600">{message}</p>
    </div>
  </div>
);
